...
// substituído pelo `Snapshot` genérico do framework: cada subsistema
// (`Reactor::new()`, por enquanto) recebe um `&Snapshot` já carregado e
// busca só as chaves que interessam pra ele, por nome — do mesmo jeito que
// já faz com `StateRegistry`.
pub mod disturbance;
pub mod model;
pub mod physics;
pub mod subsystems;
