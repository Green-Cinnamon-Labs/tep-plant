...
/** Os 7 subsistemas químicos do TEP (Reactor/Separator/Stripper/Compressor/
 ...
 `actuators` (plural — mais de um tipo no mesmo arquivo: `Valve`/`Agitator`)
 mora aqui também: são componentes físicos reais da planta, com estado e
 derivada próprios, igual aos 7 acima — a diferença é só que também
 implementam `monjolo::actuator::Actuator` (aceitam comando de fora).
*/
pub mod actuators;
pub mod compressor;
pub mod flows;
