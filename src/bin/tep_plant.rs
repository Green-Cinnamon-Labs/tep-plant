...
aplicação, não do modelo. `model::build_tep` não cabe direto em `set_model()` como ponteiro de
função (recebe `&Snapshot` também), por isso a closure abaixo.
...
fn main() {
    ...
    simulation.set_model(move |registry| model::build_tep(registry, &initial));
